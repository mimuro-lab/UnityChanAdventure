...
class Bullet
{
	...
	shared_ptr<AnimationMoveDamageObj> animationMove;
...
public:
	Bullet(shared_ptr<Stage> _stage, int init_x, int init_y, int initVel, bool isDireRight) {
		...
		animationMove = make_shared<AnimationMoveDamageObj>(initVel, 0, isDireRight);
	...
	void update(std::shared_ptr<Stage> _stage, Dimention shiftingStageVel);
